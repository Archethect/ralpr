...
2. **Select**: `RALPR_SCRIPTS/ralpr select --phase refactor` (or use provided `--pr N`)

3. **Claim PR**: `RALPR_SCRIPTS/ralpr claim pr <N>`

4. **Setup Worktree**: `RALPR_SCRIPTS/ralpr refactor --pr <N>`
...
9. **Get Confidence**: Ask agent for score (0-100) → `skill_confidence`

10. **Quality Gates**: Run tests, lint, and typecheck (use project's package manager)

11. **Push + CI**: Commit, push, `RALPR_SCRIPTS/ralpr ci wait`
...
13. **Set Label**: See @comment-formats.md
...
15. **Write Comment**: See @comment-formats.md
